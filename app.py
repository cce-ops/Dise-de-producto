...
if proveedor == "Google Gemini":
  ...
  modelos_disponibles = {
      "Gemini 3.8 Flash (Más inteligente, flujos complejos)": (
          "gemini-3.8-flash"
      ),
  ...
  seleccion_modelo = st.sidebar.selectbox(
      "Selecciona el modelo Gemini:", list(modelos_disponibles.keys()), index=0
  )
  ...
  api_key = st.sidebar.text_input(
      "NVIDIA API Key",
      type="password",
      help="Obtén tu clave gratis en build.nvidia.com",
  )
  modelos_disponibles = {
  ...
  seleccion_modelo = st.sidebar.selectbox(
      "Selecciona el modelo NVIDIA:", list(modelos_disponibles.keys()), index=0
  )
  model_name = modelos_disponibles[seleccion_modelo]
  ...
  api_key = st.sidebar.text_input(
      "OpenRouter API Key",
      type="password",
      help="Obtén tu clave gratis en openrouter.ai",
  )
  modelos_disponibles = {
      "Nemotron 3 Ultra (OpenRouter Gratis)": (
          "nvidia/nemotron-3-ultra-550b-a55b:free"
      ),
      "Llama 3.3 70B (OpenRouter Gratis)": (
  ...
  try:
    if proveedor == "Google Gemini":
      ...
      model = genai.GenerativeModel(
          model_name=model_name, system_instruction=prompt_sistema
      )
...
with tab_pds:
  ...
  st.write(
      "Introduce una especificación para comprobar si es medible y"
      " verificable."
  )
  pds_input = st.text_area(
      "Requisito PDS:",
      placeholder=(
          "Ej: La estructura soportará una carga vertical de al menos 2,0"
          " kN..."
      ),
  )
...
with tab_amfe:
  ...
  if st.button("Calcular Riesgos AMFE"):
    df_amfe_edit["NPR Calculado"] = (
        df_amfe_edit["Severidad (S)"]
        * df_amfe_edit["Ocurrencia (O)"]
        * df_amfe_edit["Detección (D)"]
    )
    condiciones = [
    ...
    df_amfe_edit["Sugerencia Sistema"] = np.select(
...
with tab_qfd:
  ...
  with col_ren:
    ...
    if comos_actuales:
      como_a_renombrar = st.selectbox(
          "Columna a modificar:", comos_actuales
      )
      nuevo_nombre = st.text_input(
          "Nuevo nombre:", placeholder="Ej: Nivel sonoro (dB)", key="ren_como"
      )
  ...
  with col_del:
    ...
    if comos_actuales:
      como_a_eliminar = st.selectbox(
          "Columna a borrar:", comos_actuales
      )
      if st.button("🗑️ Eliminar Columna"):
        if len(comos_actuales) > 1:
          ...
          st.error(
              "⚠️ Debe quedar al menos una columna CÓMO en la matriz."
          )
  ...
  config_qfd = {
      "Necesidades (QUÉ)": st.column_config.TextColumn(
          "Necesidades (QUÉ)",
          help=(
              "Escribe la necesidad en el lenguaje del usuario (Ej: 'Que sea"
              " seguro')"
          ),
      ),
  ...
  if st.button("Verificar Cálculos QFD y Generar Top-5"):
    try:
      ...
      for col in comos_cols:
        ...
        if not all(valores_relacion.isin([0, 1, 3, 9])):
          ...
          st.warning(
              ...
              " distintos a 0, 1, 3 o 9. La metodología exige usar esta escala"
              " específica."
          )
      ...
      if not alertas:
        st.success(
            "✅ Multiplicaciones verificadas con éxito. Escala 9-3-1-0"
            " respetada."
        )

      st.write(
          "**Ranking Técnico (Priorización de características para el"
          " diseño):**"
      )
      ...
      st.error(
          ...
          f" columnas numéricas. Detalle: {e}"
      )
